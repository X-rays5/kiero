...
#if KIERO_SUPPORT_D3D10 || KIERO_SUPPORT_D3D10_1
#include <dxgi.h>
#include <d3d10_1.h>
#include <d3d10.h>
...
#if KIERO_USE_POLYHOOK2
static std::unordered_map<int, std::unique_ptr<PLH::Detour>> g_detours;
#endif
  ...
  if (_renderType != RenderType::None) {
    if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12) {
      ...
      if (_renderType == RenderType::D3D9) {
        ...
        if ((direct3D9 = ((LPDIRECT3D9(__stdcall*)(uint32_t)) (Direct3DCreate9))(D3D_SDK_VERSION)) == nullptr) {
          ::DestroyWindow(window);
          ::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
          return Status::UnknownError;
        }
        ...
        HMODULE libDXGI;
        HMODULE libD3D10;
        if ((libDXGI = ::GetModuleHandle(KIERO_TEXT("dxgi.dll"))) == nullptr || (libD3D10 = ::GetModuleHandle(KIERO_TEXT("d3d10_1.dll"))) == nullptr) {
          ::DestroyWindow(window);
          ::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
          return Status::ModuleNotFoundError;
        }
        ...
        void* D3D10CreateDeviceAndSwapChain;
        if ((D3D10CreateDeviceAndSwapChain = ::GetProcAddress(libD3D10, "D3D10CreateDeviceAndSwapChain1")) == nullptr) {
          ::DestroyWindow(window);
          ::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
          return Status::UnknownError;
        }
...
kiero::Status::Enum kiero::bind(uint16_t _index, void** _original, void* _function) {
  ...
  if (g_renderType != RenderType::None) {
    ...
    #if KIERO_ARCH_X64
    g_detours[_index] = std::move(std::make_unique<PLH::x64Detour>(reinterpret_cast<uint64_t>(target), reinterpret_cast<uint64_t>(_function), reinterpret_cast<uint64_t*>(_original)));
    #endif
    #if KIERO_ARCH_X86
    g_detours[_index] = std::move(std::make_unique<PLH::x86Detour>(reinterpret_cast<uint64_t>(target), reinterpret_cast<uint64_t>(_function), reinterpret_cast<uint64_t*>(_original));
    #endif

    if (!g_detours[_index]->hook()) {
      return Status::UnknownError;
    }
    #endif
